...
var parse_accept_language = function(acceptHeader, supportedLanguages) {

    let acceptHeaderLanguages = acceptHeader.split(',');
    let possibleLanguages = [];
    let isWeighted = false;

    acceptHeaderLanguages.forEach(element => {
        element = element.trim();
        //part1
        // if (supportedLanguages.indexOf(element) !== -1) {
        //     possibleLanguages.push(element);
        // }
        //part2
        ...
        if (filteredArray.length > 0) {
            // possibleLanguages.push(...filteredArray);
            //part 3
            filteredArray.forEach(el => {
                if (!possibleLanguages.includes(el)) {
                    possibleLanguages.push(el);
                }
            });
        }
    });
...
//console.log(parse_accept_language("fr-FR, fr-CA, fr", ["fr-FR", "fr-CA", "fr-BG"]));


module.exports = parse_accept_language;